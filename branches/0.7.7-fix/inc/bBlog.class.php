...
class bBlog extends Smarty {
	...
	**********************************************************************/
	...
	// With backports from 0.8-svn
		...
		$_SESSION['checksum'] = md5($result->nickname.$result->password.BBLOGID); // Identity of the session
                return $user_id;
            } else {
	...
	/* Backported from 0.8-svn */
        function admin_logged_in() {
		$query="SELECT `id`,`nickname`,`password` FROM `".T_AUTHORS."`
		WHERE (`nickname`='".my_addslashes(@$_SESSION['nickname'])."')
		AND (`password`='".my_addslashes(@$_SESSION['password'])."')";
